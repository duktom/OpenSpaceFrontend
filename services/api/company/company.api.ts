...
import {
  CompanyDtoToEntitySchema,
  RegisterCompanyResponseSchemaDtoToData,
...
export const getCompanyById = async (params: GetCompanyByIdParams) => {
  const { id } = GetCompanyByIdParamsSchema.parse(params);
  const res = await apiClient.get<GetCompanyByIdResponse>(`/company/profile/${id}`);
  return CompanyDtoToEntitySchema.parse(res.data);
};

export const getAllCompanies = async () => {
  const res = await apiClient.get<GetAllCompaniesResponse>('/company');
