...
import { RegisterCompanyCredentialsSchema } from '@/types/backend/account/register-company';
import { RegisterUserCredentialsSchema } from '@/types/backend/account/register-user';
...
const FORM_SCHEMAS = {
  ...
  registerUser: RegisterUserCredentialsSchema,
  registerCompany: RegisterCompanyCredentialsSchema,
...
const useAuthSubmitAction = () => {
  ...
  return (formMode: FormMode, formData: FormValues): Promise<string | null> => {
    if (formMode === 'login') {
      const data = LoginCredentialsSchema.parse(formData);
      return login(data);
    }
    if (formMode === 'registerUser') {
      const data = RegisterUserCredentialsSchema.parse(formData);
      return registerUserThenLogin(data);
    }
    if (formMode === 'registerCompany') {
      const data = RegisterCompanyCredentialsSchema.parse(formData);
